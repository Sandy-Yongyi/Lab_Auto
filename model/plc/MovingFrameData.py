...
class SendMovingFrameData(PlcFrame):
    ...
    BYTE_ORDER = _BYTE_ORDER
    FRAME_SIZE: ClassVar[int] = 308              # 固定帧长度(字节), 0表示不填充，需减掉开始头和结束头的长度
    ...
    AxisList: Annotated[list[AxisData], Repeat(48)] = field(default_factory=lambda: [AxisData() for _ in range(48)])
